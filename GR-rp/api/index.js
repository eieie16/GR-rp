...
// 数据库连接池（使用 Vercel Postgres 环境变量）
const pool = new Pool({
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (url === '/api/shop/buy' && method === 'POST') {
      ...
      const userPoints = userPointsRes.rows[0].points;
      if (userPoints < item.price) return json(res, { error: '积分不足' }, 400);
    ...
    if (url === '/api/chat/messages') {
      const msgs = await pool.query(
        'SELECT username, avatar, content, timestamp, is_admin, is_vip, title FROM chat_messages ORDER BY id DESC LIMIT 100'
      );
    ...
    if (url === '/api/chat/send' && method === 'POST') {
      ...
      await pool.query(
        'INSERT INTO chat_messages (username, avatar, content, timestamp, is_admin, is_vip, title) VALUES ($1, $2, $3, $4, $5, $6, $7)',
